...
> 此时，element已经变成了zh，也就是中文，但是常规的内容怎么根据当前语言类型显示

这里，针对英文和中文，可以提供两个不同的语言包 **`src/lang/zh.js , src/lang/en.js`**
...
当全局注册i18n的时候，每个组件都会拥有一个**`$t`**的方法，它会根据传入的key，自动的去寻找当前语言的文本，可以将左侧菜单变成多语言展示文本
