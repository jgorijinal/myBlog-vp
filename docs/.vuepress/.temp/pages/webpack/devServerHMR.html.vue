...
<h2 id="为什么要搭建本地服务器" tabindex="-1"><a class="header-anchor" href="#为什么要搭建本地服务器" aria-hidden="true">#</a> 为什么要搭建本地服务器 ?</h2>
<p>目前我们开发的代码，为了运行需要有两个操作：</p>
<ul>
<li>操作一：npm run build，编译相关的代码；</li>
<li>操作二：通过live server或者直接通过浏览器，打开index.html代码，查看效果；</li>
</ul>
<p>这个过程经常操作会影响我们的开发效率，我们希望可以做到，当文件发生变化时，可以<strong>自动的完成 编译 和 展示</strong></p>
...
<p>一个个学一下</p>
<h2 id="webpack-watch-不常用" tabindex="-1"><a class="header-anchor" href="#webpack-watch-不常用" aria-hidden="true">#</a> Webpack watch(不常用)</h2>
<p>webpack 给我们提供了 watch 模式：</p>
<ul>
<li>在该模式下，webpack依赖图中的所有文件，只要有一个发生了更新，那么代码将被重新编译；</li>
<li>我们不需要手动去运行 npm run build 指令了； (不会自动编译)</li>
</ul>
<p>如何开启watch呢？两种方式：</p>
<ul>
...
<h2 id="webpack-dev-server-掌握" tabindex="-1"><a class="header-anchor" href="#webpack-dev-server-掌握" aria-hidden="true">#</a> webpack-dev-server (掌握)</h2>
<p>上面的方式可以监听到文件的变化，但是事实上它本身是没有自动刷新浏览器的功能的：</p>
<ul>
<li>当然，目前我们可以在VSCode中使用live-server 来完成这样的功能；</li>
<li>但是，我们希望在不使用 live-server的情况下，可以具备live reloading（实时重新加载）的功能；</li>
</ul>
<p>webpack-dev-server 的主要作用 : <strong>会自动地打包, 但不会生成本地的文件, 会直接将将打包的代码放到内存里面, 然后直接搭建一个服务器</strong></p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><span class="line-number">1</span><br></div></div><p>修改配置文件, 添加一个新的 scripts 脚本
<img src="@source/.vuepress/public/images/ww3.png" alt="图片"></p>
<p>webpack-dev-server 在编译之后不会写入到任何输出文件。而是将 bundle 文件保留在内存中：</p>
<ul>
<li>事实上webpack-dev-server使用了一个库叫memfs（memory-fs webpack 自己写的）</li>
</ul>
<p>执行 <code>npm run serve</code></p>
<p><img src="@source/.vuepress/public/images/ww4.png" alt="图片"></p>
...
<p>什么是 <code>HMR</code> 呢？</p>
<ul>
<li><code>HMR</code> 的全称是 <code>Hot Module Replacement</code>，翻译 <strong>为模块热替换</strong>；</li>
<li>模块热替换是指在 应用程序运行过程中，替换、添加、删除模块，而无需重新刷新整个页面；</li>
</ul>
<p><code>HMR</code> 通过如下几种方式，来提高开发的速度：</p>
<ul>
<li><strong>不重新加载整个页面</strong>，<strong>这样可以保留某些应用程序的状态不丢失</strong></li>
<li><strong>只更新需要变化的内容，节省开发的时间；</strong></li>
<li><strong>修改了css、js源代码，会立即在浏览器更新</strong>，相当于直接在浏览器的 devtools 中直接修改样式；</li>
</ul>
<p>如何使用 <code>HMR</code> 呢？</p>
<ul>
<li>默认情况下，webpack-dev-server <strong>已经支持</strong> <code>HMR</code>，我们只需要开启即可；</li>
<li>在不开启 <code>HMR</code> 的情况下，当我们修改了源代码之后，整个页面会自动刷新，使用的是 live reloading</li>
</ul>
<h3 id="开启-hmr" tabindex="-1"><a class="header-anchor" href="#开启-hmr" aria-hidden="true">#</a> 开启 HMR</h3>
<p>修改 webpack 的配置 (<strong>默认会设置为 true</strong>)</p>
<p><img src="@source/.vuepress/public/images/ww5.png" alt="图片"></p>
<p>浏览器可以看到如下效果
<img src="@source/.vuepress/public/images/ww6.png" alt="图片"></p>
<p><strong>但是你会发现，当我们修改了某一个模块的代码时，依然是刷新的整个页面：</strong></p>
<ul>
<li>这是因为我们需要去 main.js主入口文件 <strong>手动地指定</strong>哪些模块发生更新时，进行 HMR</li>
</ul>
...
<p>有一个问题：在开发其他项目时，我们是否需要经常手动去写入 module.hot.accpet 相关的API呢？</p>
<ul>
<li>比如开发Vue、React项目，我们修改了组件，希望进行热更新，这个时候应该如何去操作呢？</li>
<li>事实上社区已经针对这些有很成熟的解决方案了：</li>
<li>比如vue开发中，我们使用 vue-loader，此 loader 支持 vue 组件的 HMR，提供开箱即用的体验；</li>
<li>比如react开发中，有 React Hot Loader，实时调整 react 组件（目前 React 官方已经弃用了，改成使用react-refresh）；</li>
</ul>
<p>接下来分别对 React、Vue 实现一下HMR功能</p>
<!-- ### React 的 HMR
...
<p>Vue的加载我们需要使用vue-loader，而vue-loader加载的组件默认会帮助我们进行HMR的处理。</p>
<p>安装加载 vue 所需要的依赖：</p>
...
<h2 id="hotonly、host-配置" tabindex="-1"><a class="header-anchor" href="#hotonly、host-配置" aria-hidden="true">#</a> hotOnly、host 配置</h2>
<p>hotOnly是当代码编译失败时，是否刷新整个页面：</p>
<ul>
<li>默认情况下当代码编译失败修复后，我们会重新刷新整个页面；</li>
<li>如果不希望重新刷新整个页面，可以设置 hotOnly 为 true；</li>
</ul>
<p>host 设置主机地址：</p>
<ul>
...
<ul>
<li><strong>localhost</strong>：本质上是一个域名，通常情况下会被解析成127.0.0.1;</li>
<li><strong>127.0.0.1</strong>：回环地址(Loop Back Address)，表达的意思其实是我们主机自己发出去的包，直接被自己接收;
<ul>
...
<li><strong>0.0.0.0</strong>：监听IPV4上所有的地址，再根据端口找到不同的应用程序;
<ul>
...
<p>port 设置监听的端口，默认情况下是 <code>8080</code></p>
<p>open是否打开浏览器：</p>
<ul>
<li>默认值是 <code>false</code>，设置为 <code>true</code> 会打开浏览器；</li>
